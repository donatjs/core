...
const web = {
    ...
    // -------------------------------------------------------
    // Editor Tools (dedup: satu versi final masing-masing)
    // -------------------------------------------------------
    ...
    resolveEditor: function() {
        const key = (typeof pageFiles !== 'undefined' && pageFiles[0]) || Object.keys(pages)[0];
        return [{
            section: 'editor',
    ...
    updateDataset: function() {
        try {
            ...
            const key = web.gebi('page-select')?.value
...
// ============================================================
// COMPONENTS — Semua fungsi render UI
// ============================================================
...
const components = {
     ...
     * [PATCH CERT-FIX] handler 'form:validate-cert' sudah ada di sini
     */


    /**
         * lineRenderer: Mengubah array string menjadi HTML (Modular & DRY)
         * Ditambahkan handler untuk mendeteksi Code Block (```) standar Markdown
         */
        lineRenderer: (lines = [], context = {}) => {
            ...
            const renderedLines = data.map(line => {
                ...
                // 2. Jika sedang di dalam area Code Block, bypass parsing komponen lain (return teks mentah + newline)
                if (inCodeBlock) {
                    // Encode HTML entities sederhana agar tag <script> dll tidak dieksekusi browser sebagai DOM asli
...
<div class="sv-wrap" id="${uid}">
  ...
  <div class="sv-nav">
    ...
    // [DEDUP] Satu versi editor final dengan page-select
    editor: (d) => `
        <div class="row page">
            <div class="row">
